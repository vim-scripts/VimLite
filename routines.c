...
extern int qaculate(const char *expr);
/* calculate c integer expression
 * return 0 if there is any errors of expr  */
extern int calculateIntegerExpression (const char *expr)
{
	int n;
	vString *s = vStringNewInit(expr);
	StrCharsReplaceMod(s->buffer, "LlUu", ' ');
