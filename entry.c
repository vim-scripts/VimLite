...
#define sep (first ? (first = FALSE, separator) : empty)
	...
	if (Option.extensionFields.signature  &&
			tag->extensionFields.templatePara != NULL)
		length += fprintf (TagFile.fp, "%s\ttemplate:%s", sep,
				tag->extensionFields.templatePara);

	if (Option.extensionFields.signature  &&
			tag->extensionFields.texts != NULL)
		length += fprintf (TagFile.fp, "%s\ttext:%s", sep,
				tag->extensionFields.texts);
