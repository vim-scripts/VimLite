...
static void handlePPDefine(void)
{
	...
	if ( c == '(' )
	{
		...
		PPTokenArgs->length = strlen(PPTokenArgs->buffer);
	}
	else
	{
		fileUngetc(c);
	...
	PPValue->length = strlen(PPValue->buffer);
...
static boolean handlePPIfndef(void)
{
	return !handlePPIfdef(); /* just reverse the result */
}
...
static boolean handlePPIf(void)
{
	...
	expr->length = strlen(expr->buffer);
