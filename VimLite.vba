...
#                   os.chdir(project.dirName)
#               except OSError:
#                   print "Change to directory of project '%s' failed" \
#                       % project.GetName()
#                   return
#               projSelConfName = matrix.GetProjectSelectedConf(wspSelConfName, 
#                                                               project.name)
...
#                   # c 的编译选项
#                   cCmpOpts = self.GetProjectCompileOptions(project.name,
#                                                            '', 2)
#                   cCmpOpts = Globals.ExpandShellCmd(
#                                               cCmpOpts[0].replace(';', ' '))
#                   includes = Globals.GetIncludesFromArgs(cCmpOpts)
#                   # cpp 的编译选项
#                   cmpOpts = self.GetProjectCompileOptions(project.name, '', 2)
#                   cmpOpts = Globals.ExpandShellCmd(
#                                               cmpOpts[0].replace(';', ' '))
#                   includes += Globals.GetIncludesFromArgs(cmpOpts)
#                   [projIncludePaths.add(i[2:]) for i in includes]
#
...
#                           projIncludePaths.add(os.path.abspath(tmpPath))
    ...
    def GetProjectPredefineMacros(self, projName, wspConfName = ''):
        ...
        if self.ctime_predefineMacros.get(projName, 0) >= self.buildMTime:
            ...
            # 合并的结果
            res = self.GetProjectCompileOptions(projName, wspConfName, 8 | 64 | 128)
            # 缓存结果（副本）
    ...
    def GetProjectCompileOptions(self, projName, wspConfName = '',
        ...
        if bldConf and not bldConf.IsCustomBuild():
            compiler = BuildSettingsST().Get().GetCompiler(
        ...
        if flags & 16:
            # 解析后的 C 编译器的包含目录
            if compiler and compiler.GetSwitch("Include"):
