...
boolean EnableCppPreprocess = TRUE; /* enable cpp macros preprocess */

/*
*   FUNCTION DEFINITIONS
*/

extern void disableCppPreprocess (void)
{
	EnableCppPreprocess = FALSE;
}

extern void enableCppPreprocess (void)
{
	EnableCppPreprocess = TRUE;
}
...
{
	const char *srcLangName;
	boolean hadCheckPPLine = FALSE;
	boolean preLineContinue = File.lineContinue;
	boolean preLineStatus = File.isPPLine;
	vString *result = NULL;
	int c;
	if (File.line == NULL)
	...
	{
		c = iFileGetc ();
		/* 检查是否预处理行，只检查一次，并且只有在上一行非续行的情况下检查 */
		if (!hadCheckPPLine && !preLineContinue)
		{
			if (!isspace (c))
			{
				hadCheckPPLine = TRUE;
	...
	if ( preLineContinue )
	{
		/* 上一行为续行情况下，此行保持上一行的状态 */
		File.isPPLine = preLineStatus;
	}

	/* 检查是否续行，倒数第一个字符是 '\n' */
	if ( result != NULL && vStringLength(result) >= 2
		 && result->buffer[result->length - 2] == '\\' )
	{
		File.lineContinue = TRUE;
	}
	else
	{
		File.lineContinue = FALSE;
	...
	srcLangName = getSourceLanguageName();
	if ( EnableCppPreprocess && result != NULL && !File.isPPLine
		 && (strcmp(srcLangName, "C++") == 0 || strcmp(srcLangName, "C") == 0) )
	{
		char szTmpBuf[BUFSIZ];
		vString *ppRes = vStringNew();
		size_t bufSize = sizeof(szTmpBuf);
		char *buf = szTmpBuf, *pszContinue = NULL;
		const char *psz = vStringValue(result); /* NOTE: result 的地址可能变了 */
		int ret = 0, ppCount = 0;
		while ( (ret = CMacro_PreProcessString(psz,
											   getCppMacrosTable(),
											   getCppGlobalMacrosTable(),
											   buf, bufSize,
											   &pszContinue)) >= 0 )
		{
			if ( ret == 0 && pszContinue == NULL )
			{
				break; /* 避免无谓的处理 */
			}

			if ( IsBufferOverflow(buf, bufSize) )
			{
				/* 缓冲区溢出，扩大一倍继续 */
				bufSize *= 2;
				if ( buf == szTmpBuf ) /* 第一次扩大，不能用 xRealloc */
				{
					buf = xMalloc(bufSize, char);
				}
				else
				{
					buf = xRealloc(buf, bufSize, char);
				}
				continue;
			}

			ppCount += ret;
			vStringCatS(ppRes, buf);
			/* 再取一行 */
			if ( pszContinue != NULL )
			{
				size_t offset = pszContinue - vStringValue(result);

				if ( vStringLength(result) >= 2
					 && result->buffer[result->length - 1] == NEWLINE
					 && result->buffer[result->length - 2] == '\\' )
				{
					/* 续行情况，剔除 "\\\n" */
					vStringChop(result);
				...
				psz = vStringValue(result) + offset;
				if ( c == EOF )
				{
					/* 到达尾部都没有找到匹配的右括号，不处理 */
					break;
	...
	if ( result != NULL )
	{
		printf("%2ld %s", getInputLineNumber(), vStringValue(result));
	}
#endif
