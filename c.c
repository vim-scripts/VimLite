...
/* a mark for split conveniently */
#define INHERITANCES_SEP    ",  "
...
typedef struct sTokenInfo {
	...
	vString*	  specifiers;	 /* (__unused__) the specifiers of the token */
...
typedef struct sStatementInfo {
	...
	vString*	templatePara;   /* template parameter of class, function */
	vString*	texts;          /* texts of the statement contain the token */
...
static vString *Template;
static boolean CollectingTemplate;
...
static void deleteToken (tokenInfo *const token)
{
	if (token != NULL)
	{
		...
		vStringDelete (token->specifiers);
		eFree (token);
	}
}
...
static void reinitStatement (statementInfo *const st, const boolean partial)
{
	...
	if (! partial)
	{
		vStringClear (st->templatePara);
		vStringClear (st->texts);
	}
							...
							vString *const texts)
{
	...
	switch (type)
	{
		...
		case TAG_PROTOTYPE:
			...
			if (vStringLength (st->templatePara) > 0)
			{
				tag->extensionFields.templatePara =
					vStringValue (st->templatePara);
			}
		case TAG_CLASS:
		case TAG_ENUM:
		case TAG_ENUMERATOR:
	...
	if (vStringLength (texts) > 0)
	{
		tag->extensionFields.texts = vStringValue (texts);
	}
}
					 ...
					 boolean isFileScope, const tagType type)
{
		...
		includeTag (type, isFileScope))
	{
		...
		vString *texts = vStringNewCopy(getCppGetcCache());
		tagEntryInfo e;
		...
		StrStripSpaceMod(texts->buffer);
		StrCompactSpace(texts->buffer);
		texts->length = strlen(texts->buffer);

		findScopeHierarchy (scope, st);
		addOtherFields (&e, type, st, scope, typeRef, texts);
...
static int skipToNonWhite (void)
{
	...
	/* Equivalent to replace serial spaces to ' ' */
	if (CollectingSignature && found)
		vStringPut (Signature, ' ');
	if (CollectingTemplate && found)
		vStringPut (Template, ' ');
#endif
...
static void readIdentifier (tokenInfo *const token, const int firstChar)
{
	...
	do
	{
		...
		c = cppGetc ();
			 ...
			 || ((isLanguage (Lang_java) || isLanguage (Lang_csharp)) 
				 && (isHighChar (c) || c == '.')));
...
static void setAccess (statementInfo *const st, const accessType access)
{
	if (isMember (st))
	{
		if (isLanguage (Lang_cpp))
		{
			...
			{
				reinitStatement (st, FALSE);
				clearCppGetcCache (); /* MOD by fanhe */
			}
			else
				cppUngetc (c);
...
static void addParentClass (statementInfo *const st, tokenInfo *const token)
{
		...
		vStringLength (st->parentClasses) > 0)
	{
		...
		vStringCatS(st->parentClasses, INHERITANCES_SEP);
	}
	vStringCat (st->parentClasses, token->name);

	if ( vStringLength (Template) > 0 )
	{
		vStringCat (st->parentClasses, Template);
		vStringClear (Template);
	}
}
...
static void skipStatement (statementInfo *const st)
{
	...
	skipToOneOf (";");
	clearCppGetcCache (); /* 根据这个函数名称，这样做应该是安全的 */
}
...
static void processToken (tokenInfo *const token, statementInfo *const st)
{
	switch (token->keyword)        /* is it a reserved word? */
	{
		...
		case KEYWORD_TYPEDEF:
			reinitStatement (st, FALSE); // TODO: Process typedef statement
		...
		case KEYWORD_TEMPLATE: /* collect template */
		{
			int c = skipToNonWhite ();
			if (c == '<')
			{
				...
				skipToMatch ("<>");
				CollectingTemplate = FALSE;
				vStringCat (st->templatePara, Template);
				vStringClear (Template);
			}
			break;
		}
...
static void processAngleBracket (void)
{
	...
	if (c == '>') {
		...
		skipToMatch ("<>");
#else
		/* 人肉处理 '<'，如果是模版的话，遇到 '>' 前只可能是 '\w|,|::' */
		while ( matchLevel > 0 && (c2 = skipToNonWhite ()) != EOF )
		{
			if ( isident (c2) || c2 == ',' || c2 == ':' )
			{
				/* 进入这个分支就表示一直是模版 */
			...
			{
				/* 肯定不是模版，忽略处理 */
				cppUngetc (c2); /* TODO: 应该还原一系列字符，但是暂时无法实现 */
				break;
			}

			/* 这里要做在 skipToMatch() 要做的事，主要是下面几行
			 * 放到最后面是因为 cppUngetc() */
			if (CollectingSignature)
				vStringPut (Signature, c2);
			if (CollectingTemplate)
				vStringPut (Template, c2);
		}
		if (c2 == EOF)
			longjmp (Exception, (int) ExceptionFormattingError);
#endif
...
static void deleteStatement (void)
{
	...
	vStringDelete (st->templatePara);      st->templatePara = NULL;
	vStringDelete (st->texts);             st->texts = NULL;
	eFree (st);
...
static void tagCheck (statementInfo *const st)
{
	...
	switch (token->type)
	{
		case TOKEN_NAME:
			if (insideEnumBody (st))
				qualifyEnumeratorTag (st, token); /* NOTE: 枚举器后面的字符串没法收集 */
			break;
#if 0
		case TOKEN_PACKAGE:
		...
		case TOKEN_BRACE_OPEN:
					...
					st->declaration == DECL_PROGRAM)
			{
				...
				else
				{
					...
					/* patch __anon */
					vStringCatS (st->blockName->name, "_");
					vStringCatS (st->blockName->name,
								 baseFilename(getSourceFileName()));
					p = vStringValue (st->blockName->name);
					for ( ; *p != '\0'; p++)
					{
						if ( !isalnum(*p) )
					...
					vStringCopy (anonToken->name, st->blockName->name);
					anonToken->type = st->blockName->type;
					anonToken->keyword = st->blockName->keyword;
					qualifyBlockTag (st, anonToken); /* add anonymous tag */
