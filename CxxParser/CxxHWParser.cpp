...
void DetermineVariable(std::list<CxxToken> &stmtToks, CxxScope &scope)
{
    ...
    // 跳过 *, &
    // TODO
    if ( tokRdr.PeekToken().id == CXX_OP_LParen )
    {
         tok = tokRdr.GetToken();
         if ( tokRdr.PeekToken().id != CXX_OP_Mul ) // 这里检查失败就恢复继续
         {
         ...
         // 这里开始不允许失败了，否则一律视为错误
         tokRdr.GetToken();
         tok = tokRdr.GetToken(); // 单词
         if ( tok.IsWord() && tokRdr.PeekToken().id == CXX_OP_RParen
              && (tokRdr.GetToken(), tokRdr.PeekToken().id == CXX_OP_LParen) )
         {
             // 括号后面的内容暂时就不检查了
             var.name = tok.text;
