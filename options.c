...
static optionDescription LongOptionDescription [] = {
 ...
 {0,"  -m   Include all condition compilation branches."},
...
static void parseLongOption (cookedArgs *const args, const char *item)
{
	...
	if (equal == NULL)
	{
		args->item = eStrdup (item); /* NOTE: ugly1. */
		args->parameter = "";
	}
	else
	{
		const size_t length = equal - item;
		args->item = xMalloc (length + 1, char); /* NOTE: ugly1. */
...
static void cArgRead (cookedArgs *const current)
{
	...
	if (! argOff (current->args))
	{
		...
		Assert (item != NULL);
		if (current->longOption) /* NOTE: ugly1. */
		{
			eFree (current->item);
			current->item = NULL;
		}
