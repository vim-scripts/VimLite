...
# NOTE: 需要添加 inotify，因为当两个不同的工作区打开同一个项目文件的时候，
#       项目的构建设置可能会不同于工作区选择的，因为缓存的机制，
#       所以，要确保同时只引用一份项目文件，否则后果自负

BUILDER_NAME = 'Another GNU makefile for g++/gcc'
...
class BuilderGnuMake2(Builder):
    ...
    def Export2(self, projects, confToBuild = '', force = False):
        ...
        try:
            t1 = GetMTime(wspMakefile)
            f = open(wspMakefile, 'wb')
            f.write(text)
            f.close()
            for i in range(10): # 最多等待 0.1 秒
                # 需要等待文件确实写到硬盘
                if GetMTime(wspMakefile) > t1:
                    break
                else:
                    time.sleep(0.01)
        except IOError:
    ...
    def GenerateMakefile(self, project, confToBuild, force):
        ...
        # 添加 objects 变量
        text += self.CreateObjectList(project, confToBuild)

        # 添加命令变量
        text += self.CreateCommandVariables()
        ...
        # 执行顺序: MakeDirStep, PreBuild, $(OutputFile): $(Objects), PostBuild
        # 添加顺序是逆顺序
        ...
        # 添加 MakeDirStep 目标
        text += self.CreateMakeDirsTarget(bldConf, 'MakeDirStep')
    ...
    def CreateCommandVariables(self):
        text = ''
        text += '\n'

        # C 源文件编译命令
        text += 'CCompileCommand = %s\n' % \
                '$(C_CompilerName) $(SourceSwitch) $< $(C_CmpOptions) $(ObjectSwitch)$@ $(IncludePath)\n'
        # C++ 源文件编译命令
        text += 'CXXCompileCommand = %s\n' % \
                '$(CompilerName) $(SourceSwitch) $< $(CmpOptions) $(ObjectSwitch)$@ $(IncludePath)\n'
        # TODO
        # C 预处理命令
        # C++ 预处理命令
        # C 依赖生成命令
        ...
        if bldConf and not cleanOnly:
            ...
            if preCmpHeader:
                #makeCommand += basicMakeCommand + ' ' + preCmpHeader + '.gch' \
                        #+ ' && '
                lMakeCommands.append('%s %s.gch' 
                                     % (basicMakeCommand, preCmpHeader))
